...
1. 外部数据卷场景下连续 `6` 轮切换与恢复后，集群仍可自动收敛。
2. 双节点同时断电后，先启动一侧、再启动另一侧的恢复路径可用。
3. 大数量持续写入期间，主备数据一致性校验持续通过。
4. WAL 归档、`pg_receivewal` 接收和自动清理可同时工作。

### 最近一次完整回归结果（外部数据卷 + 大写入 + WAL 自动清理）
...
1. `R1` 正常停止：`pg-node1 -> pg-node2`，切换 `26.06s`，旧主回归 `5.86s`
2. `R2` 突然断电：`pg-node2 -> pg-node1`，切换 `26.24s`，旧主回归 `40.83s`
3. `R3` 双节点同时断电恢复：首节点恢复 `11.57s`，第二节点回归 `4.83s`
4. `R4` 正常停止：`pg-node1 -> pg-node2`，切换 `26.31s`，旧主回归 `4.74s`
5. `R5` 突然断电：`pg-node2 -> pg-node1`，切换 `27.22s`，旧主回归 `42.19s`
6. `R6` 双节点同时断电恢复：首节点恢复 `11.74s`，第二节点回归 `4.99s`

本轮验证结果如下：

1. 单节点故障切换平均耗时 `26.46s`
2. 双节点同时断电后，首节点平均恢复 `11.66s`，第二节点平均回归 `4.91s`
3. `VIP` 最终都会收敛到当前 `primary`，但切换窗口内存在短暂空窗，不能表述为“始终跟随”
4. 主备数据一致性 `6/6` 通过
5. 企业微信在正常停止和单节点断电场景验证成功；双节点同时断电恢复场景未验证为始终成功
6. WAL 自动清理在 `6` 轮中触发了 `5` 轮，共删除 `20` 个 WAL 段，且未破坏恢复链路
